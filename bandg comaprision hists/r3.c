void r3()
{
    ...
    TH1F *sh = new TH1F("sh", "Electron_pt_2_signal_histogram", 39, 0, 296.9);
    TH1F *bh = new TH1F("bh", "Electron_pt_2_background_histogram", 39, 0, 296.9);

    st->Draw("Electron_pt_2>>sh");
    bt->Draw("Electron_pt_2>>bh");

    TCanvas *c = new TCanvas("c", "branch_3_Histogram");
    bh->Draw();
    sh->Draw("same");
