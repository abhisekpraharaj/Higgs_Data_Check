void r4()
{
    ...
    TH1F *sh = new TH1F("sh", "dPhim1e1_signal_histogram", 35, -3, 5);
    TH1F *bh = new TH1F("bh", "dPhim1e1_background_histogram", 35, -3, 5);

    st->Draw("dPhim1e1>>sh");
    bt->Draw("dPhim1e1>>bh");

    TCanvas *c = new TCanvas("c", "branch_4_Histogram");
    bh->Draw();
    sh->Draw("same");
