void r6()
{
    ...
    TH1F *sh = new TH1F("sh", "dPhim2e1_signal_histogram", 35 ,-3 ,5);
    TH1F *bh = new TH1F("bh", "dPhim2e1_background_histogram", 35 ,-3 ,5);

    st->Draw("dPhim2e1>>sh");
    bt->Draw("dPhim2e1>>bh");

    TCanvas *c = new TCanvas("c", "branch_6_Histogram");
    bh->Draw();
    sh->Draw("same");
