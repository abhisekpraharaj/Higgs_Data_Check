void r8()
{
    ...
    TH1F *sh = new TH1F("sh", "Electron_eta_1_signal_histogram", 35 ,-3 ,5);
    TH1F *bh = new TH1F("bh", "Electron_eta_1_background_histogram", 35 ,-3 ,5);

    st->Draw("Electron_eta_1>>sh");
    bt->Draw("Electron_eta_1>>bh");

    TCanvas *c = new TCanvas("c", "branch_8_Histogram");
    bh->Draw();
    sh->Draw("same");
